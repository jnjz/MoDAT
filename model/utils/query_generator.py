...
from model.utils import build_fusion_block
...
class EncoderDecoderGenerator(nn.Module):
    def __init__(self, num_layers_decoder=3, 
                 query_num=300, embed_dim=256, num_heads=8, hidden_dim=1024,
                 early_fusion=None):
        super().__init__()
        self.query_num = query_num
        self.embed_dim = embed_dim
        if early_fusion:
            self.early_fusion = build_fusion_block(**early_fusion)
        else:
        ...
        self.conv = nn.Conv2d(num_layers_decoder, 1, kernel_size=1)
                
    def forward(self, x, audio_feat):
        '''
        x: len=3     [10, 256, 28, 28], [10, 256, 14, 14], [10, 256, 7, 7]   multi-scale feature map after unifying channels
        audio_feat: (10, 1, 256)
        '''
        if self.early_fusion:
            kv = self.early_fusion(x, audio_feat)
        else: 
            kv=audio_feat
        ...
        querys = torch.stack(querys, dim=1)   # (bs, 3, 300, 256)
        query = self.conv(querys).squeeze(1)   
        return query, kv
...
def build_generator(type, **kwargs):
    ...
    elif type == 'EncoderDecoderGenerator':
        return EncoderDecoderGenerator(**kwargs)
    else:
        raise ValueError
